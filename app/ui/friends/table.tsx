...
import {
  FriendsTableType,
  FormattedFriendsTable,
} from '@/app/lib/definitions';

export default async function FriendsTable({
  friends,
}: {
  friends: FormattedFriendsTable[];
}) {
  return (
                  ...
                  >
                    ...
                    <div className="flex w-full items-center justify-between border-b py-5">
                      <div className="flex w-1/2 flex-col">
                        ...
                        <p className="font-medium">{friend.total_pending}</p>
                      </div>
                      <div className="flex w-1/2 flex-col">
                        <p className="text-xs">Paid</p>
                        <p className="font-medium">{friend.total_paid}</p>
                      </div>
                    </div>
                    <div className="pt-4 text-sm">
                      <p>{friend.total_payments} payments</p>
                    </div>
                  </div>
                ))}
              </div>
                ...
                <tbody className="divide-y divide-payne-200 text-payne-900">
                  {friends.map((friend) => (
                    <tr key={friend.id} className="group">
                      ...
                      <td className="whitespace-nowrap bg-white px-4 py-5 text-sm">
                        {friend.total_payments}
                      </td>
                      <td className="whitespace-nowrap bg-white px-4 py-5 text-sm">
                        {friend.total_pending}
                      </td>
